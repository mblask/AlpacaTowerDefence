...
public class EnemyHandler : IEnemyHandler
{
    ...
    private EnemyBehaviour _behaviour = EnemyBehaviour.Survive;
    ...
    public EnemyHandler(EnemyTemplate template, Transform enemyTransform)
    {
        IsActive = true;
    ...
    {
        _isAttacking = false;
        if (_buildingsContainer.GetContainer().childCount == 0)
            return;
